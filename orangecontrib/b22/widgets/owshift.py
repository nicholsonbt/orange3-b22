...
from AnyQt.QtWidgets import QWidget, QPushButton, \
    ...
    QToolTip, QGraphicsRectItem, QLineEdit, QApplication, QHBoxLayout, QLabel
from AnyQt.QtGui import QColor, QKeySequence, QPainter, QBrush, QStandardItemModel, \
    QStandardItem, QLinearGradient, QPixmap, QIcon, QDoubleValidator
...
class OWShift(OWWidget):
    ...
    offset_x = Setting(0.0)
    offset_y = Setting(0.0)
    ...
    def __init__(self):
        ...
        self.curveplot = CurvePlotHyper(self)
    ...
    def setup_visible_image_controls(self):
        self.visbox = gui.widgetBox(self.controlArea, "Shift images")
        ...
        x_offset = LineEdit(self, "offset_x", callback=self.offsetChanged, callbackOnType=True)
        x_box.addWidget(x_label)
        x_box.addWidget(x_offset)

        y_box = QHBoxLayout()
        y_label = QLabel("Y Offset:")
        y_offset = LineEdit(self, "offset_y", callback=self.offsetChanged, callbackOnType=True)
        y_box.addWidget(y_label)
        y_box.addWidget(y_offset)

        self.visbox.layout().addLayout(x_box)
        self.visbox.layout().addLayout(y_box)
    ...
    def offsetChanged(self):
        data = self.getOffsetData()
        self.imageplot.set_data(data)
        self.init_visible_images(data)

        # Not entirely sure why this is needed, but it is.
        if data is not None:
            self.commit.deferred()
...
if __name__ == "__main__":  # pragma: no cover
    ...
    WidgetPreview(OWShift).run(set_data=collagen)